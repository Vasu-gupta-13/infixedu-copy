...
import 'package:infixedu_copy/main.dart';
import 'package:infixedu_copy/screens/chat.dart';
...
import 'package:infixedu_copy/utils/models/received_sms_model.dart';
import 'package:infixedu_copy/utils/models/user_notification.dart';
import 'package:infixedu_copy/utils/server/about.dart';
import 'package:infixedu_copy/utils/server/logout_service.dart';
...
class _HomeState extends State<Home> {
    ...
    didReceiveLocalNotificationSubject.stream
      ...
      await showDialog(
        ...
        builder: (BuildContext context) => CupertinoAlertDialog(
          title: receivedNotification.title!.isNotEmpty
              ? Text(receivedNotification.title!)
              : null,
          content: receivedNotification.body!.isNotEmpty
              ...
              GridView.builder(
                ...
                itemBuilder: (context, index) {
                  return CustomWidget(
                    ...
                    headline: widget.title[index],
                    icon: widget.image[index],
                  );
                },
              ),
              SizedBox(
                                            ...
                                            child: Dismissible(
                                              ...
                                              onDismissed: (direction) async {
                                                var response = await http.get(
                                                    Uri.parse(InfixApi.readMyNotifications(int.parse(id) as String,snapshot.data!.userNotifications[index].id)),
                                                ...
                                                if (response.statusCode ==
                                                  ...
                                                  if (status == true) {
                                                    setState(() {
                                                      ...
                                                      snapshot.data!.userNotifications.removeAt(index);
                                                    });
                                                  }
                                                } else {
                                                  debugPrint(
                                                  ...
                                                  Column(
                                                    ...
                                                    children: [
                                                      Text(
                                                        ...
                                                        textAlign:
                                                        TextAlign.end,
                                                        style: Theme.of(context).textTheme.headlineSmall!.copyWith(fontSize:ScreenUtil().setSp(13),),
                                                      ),
                                                      Text(
                                                        timeago.format(item.createdAt),textAlign:TextAlign.end,style: Theme.of(context).textTheme.headlineSmall!.copyWith(fontSize:ScreenUtil().setSp(12),
                                                        ),
                                                      ),
                                                      const SizedBox(
                                                        height: 12,
  ...
  Future<String> getImageUrl(String email, String password, String rule) async {
    ...
    if (response.statusCode == 200) {
     dynamic user = jsonDecode(response.body) as Map;
      if (rule == '2') {
  ...
  Future<int> getNotificationCount(int id) async {
    ...
    if (response.statusCode == 200) {
      dynamic notifications = jsonDecode(response.body) as Map;
